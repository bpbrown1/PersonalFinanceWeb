...
describe('CategoriesApiService', () => {
  ...
  beforeEach(() => {
    TestBed.configureTestingModule({ providers: [CategoriesApiService, provideHttpClient(withInterceptors([apiErrorInterceptor])),
      provideHttpClientTesting(), { provide: API_BASE_URL, useValue: 'http://localhost:8080/api/v1' }] });
  ...
  it('retrieves categories using the requested lifecycle filter', async () => {
    ...
    expect(request.request.method).toBe('GET'); request.flush([categoryFixture()]);
    await expect(result).resolves.toEqual([categoryFixture()]);
  });

  it('creates a category using the documented contract', async () => {
    const body: CreateCategoryRequest = { name: 'Groceries', applicability: 'expense' };
    const result = firstValueFrom(service.create(body));
    const request = http.expectOne('http://localhost:8080/api/v1/categories');
    expect(request.request.method).toBe('POST'); expect(request.request.body).toEqual(body); request.flush(categoryFixture());
    await expect(result).resolves.toEqual(categoryFixture());
  });
  ...
  it('retrieves and updates safely encoded category identifiers', async () => {
    ...
    http.expectOne('http://localhost:8080/api/v1/categories/category%2Fwith%20space').flush(categoryFixture());
    await result;
    const updated = firstValueFrom(service.update('category/with space', { name: 'Food', applicability: 'both' }));
    const request = http.expectOne('http://localhost:8080/api/v1/categories/category%2Fwith%20space');
    expect(request.request.method).toBe('PATCH'); expect(request.request.body).toEqual({ name: 'Food', applicability: 'both' });
    request.flush({ ...categoryFixture(), name: 'Food', applicability: 'both' }); await updated;
  });

  it('archives and restores without exposing deletion', async () => {
    for (const action of ['archive', 'restore'] as const) {
      const result = firstValueFrom(service[action]('category-1'));
      const request = http.expectOne(`http://localhost:8080/api/v1/categories/category-1/${action}`);
      expect(request.request.method).toBe('POST'); expect(request.request.body).toEqual({}); request.flush(categoryFixture()); await result;
    }
  });
});
