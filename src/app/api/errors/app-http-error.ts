...
export type AppHttpErrorKind =
  'network' | 'validation' | 'not-found' | 'client' | 'server' | 'unexpected';
...
export class AppHttpError extends Error {
  constructor(
    ...
    readonly existingBudgetId: string | null = null,
  ) {
...
export function normalizeHttpError(error: unknown): AppHttpError {
  ...
  if (error.status === 0) {
    return new AppHttpError(
      'network',
      'Unable to reach the Personal Finance service. Check that the API is running and try again.',
      0,
      {},
      true,
      { cause: error },
  ...
  if (error.status === 400 && Object.keys(fieldErrors).length > 0) {
    return new AppHttpError(
      'validation',
      body?.error ?? 'Please correct the highlighted fields.',
      400,
      fieldErrors,
      false,
      { cause: error },
      body?.existingBudgetId ?? null,
    );
  }
  if (error.status === 404) {
    return new AppHttpError(
      'not-found',
      body?.error ?? 'The requested record could not be found.',
      404,
      {},
      false,
      { cause: error },
