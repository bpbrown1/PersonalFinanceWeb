...
export class AccountHistory implements OnInit {
  ...
  protected readonly customRangeValid = computed(() =>
    Boolean(this.customFrom() && this.customTo() && this.customFrom() <= this.customTo()),
  );
  ...
  protected readonly chartPoints = computed(() => {
    ...
    return values.map((value, index) => ({
      x:
        minTime === maxTime
          ? width / 2
          : pad + ((times[index] - minTime) / (maxTime - minTime)) * (width - pad * 2),
      y:
        minBalance === maxBalance
          ? height / 2
          : height -
            pad -
            ((balances[index] - minBalance) / (maxBalance - minBalance)) * (height - pad * 2),
      snapshot: value,
    }));
  });
  ...
  ngOnInit(): void {
    this.route.queryParamMap.subscribe((params) => {
      ...
      const selectedRange: HistoryRange =
        range === '30d' || range === '1y' || range === 'custom' ? range : '90d';
  ...
  protected load(): void {
    ...
    forkJoin({
      ...
      snapshots: this.balancesApi.history(this.accountId),
    })
      .pipe(finalize(() => this.loading.set(false)))
      .subscribe({
        next: ({ account, snapshots }) => {
  ...
  protected selectRange(range: Exclude<HistoryRange, 'custom'>): void {
    void this.router.navigate([], {
      ...
      queryParams: { range, from: null, to: null },
  ...
  protected applyCustomRange(): void {
    ...
    void this.router.navigate([], {
      relativeTo: this.route,
      queryParams: { range: 'custom', from: this.customFrom(), to: this.customTo() },
      queryParamsHandling: 'merge',
    });
  }
  ...
  protected findAsOf(): void {
    ...
    this.asOfResult.set(null);
    this.asOfError.set(null);
    const instant = new Date(`${this.asOfDate()}T23:59:59.999`).toISOString();
    this.balancesApi
      .asOf(this.accountId, instant)
      .pipe(finalize(() => this.asOfLoading.set(false)))
      .subscribe({
  ...
  private boundaries(): { from: number; to: number } {
    if (this.range() === 'custom' && this.customFrom() && this.customTo()) {
      return {
        from: new Date(`${this.customFrom()}T00:00:00`).getTime(),
        to: new Date(`${this.customTo()}T23:59:59.999`).getTime(),
      };
    }
    const to = Date.now();
    const days = this.range() === '30d' ? 30 : this.range() === '1y' ? 365 : 90;
