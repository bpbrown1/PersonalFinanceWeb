...
describe('AccountHistory', () => {
  ...
  let accounts: { get: ReturnType<typeof vi.fn> }; let presenter: { present: ReturnType<typeof vi.fn> }; let router: Router;

  beforeEach(async () => {
    params.next(convertToParamMap({ range:'90d' }));
    balances = { history:vi.fn().mockReturnValue(of(snapshotFixtures())), asOf:vi.fn().mockReturnValue(of({ accountId:account.id,balance:1400,effectiveAt:daysAgo(20),source:'manual' })) };
    accounts={get:vi.fn().mockReturnValue(of(account))}; presenter={present:vi.fn((error)=>error)};
    await TestBed.configureTestingModule({ imports:[AccountHistory], providers:[provideRouter([]),
      {provide:ActivatedRoute,useValue:{snapshot:{paramMap:{get:()=>account.id}},queryParamMap:params.asObservable()}},
      {provide:AccountBalancesApiService,useValue:balances},{provide:AccountsApiService,useValue:accounts},{provide:ApiErrorPresenter,useValue:presenter}] }).compileComponents();
  ...
  it('renders only recorded snapshots as an accessible trend and table', () => {
    ...
    expect(accounts.get).toHaveBeenCalledWith(account.id); expect(balances.history).toHaveBeenCalledWith(account.id);
  ...
  it('filters presets without fabricating intermediate balances and writes range state to the URL', () => {
    const fixture=TestBed.createComponent(AccountHistory); fixture.detectChanges();
    params.next(convertToParamMap({range:'30d'})); fixture.detectChanges();
    ...
    expect(router.navigate).toHaveBeenCalledWith([],expect.objectContaining({queryParams:{range:'30d',from:null,to:null}}));
  });
  ...
  it('initializes custom dates and applies after changing only one boundary', () => {
    ...
    expect(from.value).not.toBe(''); expect(to.value).not.toBe(''); expect(apply.disabled).toBe(false);
    const originalTo=to.value;
    from.value='2000-01-01'; from.dispatchEvent(new Event('input')); fixture.detectChanges();
    expect(to.value).toBe(originalTo); expect(apply.disabled).toBe(false);
    apply.click();
    expect(router.navigate).toHaveBeenCalledWith([],expect.objectContaining({queryParams:{range:'custom',from:'2000-01-01',to:originalTo}}));
  });
  ...
  it('offers retry after a history load failure', () => {
    const error=new AppHttpError('network','History unavailable',0,{},true); balances.history.mockReturnValue(throwError(()=>error));
    const fixture=TestBed.createComponent(AccountHistory); fixture.detectChanges();
    expect(presenter.present).toHaveBeenCalledWith(error); fixture.nativeElement.querySelector('app-page-state button').click();
...
function snapshotFixtures():BalanceSnapshot[] { return [80,20,1].map((days,index)=>({id:`snapshot-${index}`,accountId:'account-1',balance:1200+index*200,effectiveAt:daysAgo(days),source:index?'manual':'opening',createdAt:daysAgo(days)})); }
function accountFixture():FinancialAccount { return {id:'account-1',ownerId:'owner-1',name:'Everyday Checking',type:'checking',currency:'USD',openingDate:'2026-05-01',openingBalance:1200,currentBalance:1600,status:'active',archivedAt:null,createdAt:daysAgo(100),updatedAt:daysAgo(1)}; }
