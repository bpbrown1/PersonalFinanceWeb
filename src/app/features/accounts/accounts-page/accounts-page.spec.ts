...
describe('AccountsPage', () => {
  ...
  let api: { list: ReturnType<typeof vi.fn>; archive: ReturnType<typeof vi.fn>; restore: ReturnType<typeof vi.fn> };
  let notifications: { show: ReturnType<typeof vi.fn> };
  let presenter: { present: ReturnType<typeof vi.fn> };

  beforeEach(async () => {
    api = { list: vi.fn().mockReturnValue(of([account])), archive: vi.fn().mockReturnValue(of({ ...account, status: 'archived' })), restore: vi.fn().mockReturnValue(of(account)) };
    notifications = { show: vi.fn() }; presenter = { present: vi.fn((error) => error) };
    await TestBed.configureTestingModule({ imports: [AccountsPage], providers: [provideRouter([]),
      { provide: AccountsApiService, useValue: api }, { provide: NotificationService, useValue: notifications }, { provide: ApiErrorPresenter, useValue: presenter }] }).compileComponents();
  });
  ...
  it('requires confirmation before archive and refreshes after success', () => {
    ...
    const fixture = TestBed.createComponent(AccountsPage); fixture.detectChanges();
    fixture.nativeElement.querySelector('button.secondary').click(); fixture.detectChanges();
